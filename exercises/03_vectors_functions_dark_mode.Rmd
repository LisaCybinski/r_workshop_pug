...
min(hotdog_2019)
```
#multiline curosr hold "alt" und hochziehen, dann ändern#
...
5.2) `min()` and `max()`
```{r}
min(c(1, 20, NA, 12, 3), na.rm = TRUE)
max(c(1, 20, NA, 12, 3), na.rm = TRUE)
```


5.3) `summary()`
```{r}
summary(c(1, 20, NA, 12, 3), na.rm = TRUE)
```
...
5.5) `head()` and `tail()`
```{r}
head(c(1, 20, NA, 12, 3), na.rm = TRUE)
tail(c(1, 20, NA, 12, 3), na.rm = TRUE)
```
